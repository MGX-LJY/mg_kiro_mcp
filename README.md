...
- **四种工作模式** - Init/Create/Fix/Analyze模式处理器
...
```
mg_kiro_mcp/                           # 项目根目录 (~3500行代码)
├── server/                            # 🔥 服务器核心模块 (2100行)
│   ├── mcp-server.js                  # MCP协议服务器 (800行)
│   │                                  # HTTP + WebSocket双协议支持
│   │                                  # 中间件堆栈、连接管理、错误处理
│   ├── prompt-manager.js              # 智能提示词管理器 (400行)
│   │                                  # 缓存机制、热重载、变量替换
│   ├── mode-handler.js                # 四种工作模式处理器 (600行)
│   │                                  # Init/Create/Fix/Analyze业务逻辑
